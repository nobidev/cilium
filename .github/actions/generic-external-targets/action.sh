...
mapfile -d ' ' -t IPTARGET < <(kubectl get nodes -l cilium.io/no-schedule=true -o jsonpath='{.items[0].status.addresses[?(@.type=="ExternalIP")].address}')
mapfile -d ' ' -t IPOTHERTARGET < <(kubectl get nodes -l cilium.io/no-schedule=true -o jsonpath='{.items[1].status.addresses[?(@.type=="ExternalIP")].address}'
