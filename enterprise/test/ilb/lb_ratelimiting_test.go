...
func TestHTTPRouteRatelimiting(t *testing.T) {
	...
	testCmd := curlCmdVerbose(fmt.Sprintf("-m 2 --resolve %s:80:%s http://%s:80%s", hostName, vipIP, hostName, "/"))
	...
	{
		testCmdAdmin := curlCmdVerbose(fmt.Sprintf("-m 2 --resolve %s:80:%s http://%s:80%s", hostName, vipIP, hostName, "/admin"))
...
func TestHTTPApplicationRatelimiting(t *testing.T) {
	ctx := context.Background()
