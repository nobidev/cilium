...
func TestTCPProxyConnectionFiltering(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			testName := fmt.Sprintf("tcp-proxy-connectionfiltering-%s", tC.desc)
			...
			service := lbService(testK8sNamespace, testName, withPort(80), withTCPProxyApplication(withTCPForceDeploymentMode(isovalentv1alpha1.LBTCPProxyForceDeploymentModeT1), withTCPProxyRoute(backendPool.Name, tC.appOpt(clients))))
			...
			for _, tt := range tC.testCalls {
				...
				eventually(t, func() error {
					...
					} else if tt.blocked && (err == nil || err.Error() != "cmd failed: 28") { // -> 28: timeout
