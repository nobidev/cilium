...
func TestConnectionAggregationTCP(t *testing.T) {
	...
	defer cancel()
	r := ca.Aggregate(p1)
	assert.Equal(t, r.StateChange, aggregationpb.StateChange_new)
	...
	r = ca.Aggregate(p3)
	assert.Equal(t, r.StateChange, aggregationpb.StateChange_established)
	assert.Equal(t, r.AggregatedFlow.FirstFlow, p1)
	...
	// Different flow
	r = ca.Aggregate(p6)
	assert.Equal(t, r.StateChange, aggregationpb.StateChange_new|aggregationpb.StateChange_first_reply)

	r = ca.Aggregate(p7)
	assert.Equal(t, r.StateChange, aggregationpb.StateChange_unspec)
	assert.Equal(t, r.AggregatedFlow.FirstFlow, p1)
	r = ca.Aggregate(p8)
	assert.Equal(t, r.StateChange, aggregationpb.StateChange_closed)
	assert.Equal(t, r.AggregatedFlow.FirstFlow, p1)

	af := ca.Cache().Lookup(p2)
	assert.Equal(t, af.FirstFlow, p1)

	assert.EqualValues(t, af.Stats.Forward.NumFlows, 3)
	assert.EqualValues(t, af.Stats.Reply.NumFlows, 3)
...
func TestConnectionAggregationUDP(t *testing.T) {
	...
	assert.EqualValues(t, af.Stats.Forward.NumFlows, 1)
	assert.EqualValues(t, af.Stats.Reply.NumFlows, 1)
}
