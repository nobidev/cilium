...
// APICell provides the restapi.CiliumEnterpriseAPIAPI type, populated
...
func newAPI(p apiParams) *restapi.CiliumEnterpriseAPIAPI {
	api := restapi.NewCiliumEnterpriseAPIAPI(p.Spec.Document)
...
type ServerConfig struct {
	EnableCiliumEnterpriseAPIServerAccess []string
}
...
var (
	...
	AdminEnableFlag = "enable-cilium-enterprise-api-server-access"
)

func (cfg ServerConfig) Flags(flags *pflag.FlagSet) {
	flags.StringSlice(AdminEnableFlag, cfg.EnableCiliumEnterpriseAPIServerAccess,
		"List of cilium enterprise API APIs which are administratively enabled. Supports '*'.")
}

var SpecCell = cell.Module(
	"cilium-enterprise-api-spec",
	"cilium enterprise API Specification",
...
func newSpec(cfg ServerConfig) (*Spec, error) {
	...
	deniedAPIs, err := api.AllowedFlagsToDeniedPaths(swaggerSpec, cfg.EnableCiliumEnterpriseAPIServerAccess)
	if err != nil {
...
// NewServer creates a new api cilium enterprise API server but does not configure it
func NewServer(api *restapi.CiliumEnterpriseAPIAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
// Server for the cilium enterprise API API
type Server struct {
	EnabledListeners []string
...
func (s *Server) SetAPI(api *restapi.CiliumEnterpriseAPIAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) GetAPI() *restapi.CiliumEnterpriseAPIAPI {
	return s.api
}
...
func (s *Server) Start(cell.HookContext) (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		s.wg.Add(1)
		s.Logf("Serving cilium enterprise API at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := domainSocket.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving cilium enterprise API at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		s.wg.Add(1)
		s.Logf("Serving cilium enterprise API at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := httpServer.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving cilium enterprise API at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		s.wg.Add(1)
		s.Logf("Serving cilium enterprise API at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer s.wg.Done()
			if err := httpsServer.Serve(l); err != nil && err != http.ErrServerClosed {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving cilium enterprise API at https://%s", l.Addr())
