...
// Changes include copying some referenced variables from oauth2cli.go into this file , and renaming GetToken to AuthorizationCodeToken
...
const authCodeExchangeTimeout = 2 * time.Minute

// Config represents a config for AuthorizationCodeToken.
...
func (c *ServerConfig) isLocalServerHTTPS() bool {
	return c.LocalServerCertFile != "" && c.LocalServerKeyFile != ""
}

func (c *ServerConfig) validateAndSetDefaults() error {
...
// AuthorizationCodeToken performs the Authorization Code Grant Flow and
// returns a token received from the provider.
...
func AuthorizationCodeToken(ctx context.Context, c ServerConfig) (*oauth2.Token, error) {
	...
	ctx, cancel := context.WithTimeout(ctx, authCodeExchangeTimeout)
	defer cancel()
	code, err := receiveCodeViaLocalServer(ctx, &c)
	if errors.Is(err, context.DeadlineExceeded) {
		return nil, fmt.Errorf("timed out waiting for user to login")
	}
	if err != nil {
		return nil, fmt.Errorf("authorization error: %w", err)
	}

	c.Logger.Debug("oauth2cli: exchanging the code and token")
