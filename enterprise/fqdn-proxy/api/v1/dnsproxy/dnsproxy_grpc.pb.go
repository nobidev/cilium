...
const (
	...
	FQDNProxyAgent_SubscribeProxyStatuses_FullMethodName      = "/dnsproxy.FQDNProxyAgent/SubscribeProxyStatuses"
...
type FQDNProxyAgentClient interface {
	...
	// Returns a stream of ProxyStatuses
	SubscribeProxyStatuses(ctx context.Context, in *Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ProxyStatus], error)
...
func (c *fQDNProxyAgentClient) SubscribeProxyStatuses(ctx context.Context, in *Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ProxyStatus], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &FQDNProxyAgent_ServiceDesc.Streams[1], FQDNProxyAgent_SubscribeProxyStatuses_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[Empty, ProxyStatus]{ClientStream: stream}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FQDNProxyAgent_SubscribeProxyStatusesClient = grpc.ServerStreamingClient[ProxyStatus]
...
type FQDNProxyAgentServer interface {
	...
	// Returns a stream of ProxyStatuses
	SubscribeProxyStatuses(*Empty, grpc.ServerStreamingServer[ProxyStatus]) error
	// Returns a stream of FQDNRules from the agent to the proxy
	// Proxy should write an Empty response to ack receipt of the rule.
	// Added: v1.18-ce.
...
func (UnimplementedFQDNProxyAgentServer) SubscribeProxyStatuses(*Empty, grpc.ServerStreamingServer[ProxyStatus]) error {
	return status.Errorf(codes.Unimplemented, "method SubscribeProxyStatuses not implemented")
}
...
func _FQDNProxyAgent_SubscribeProxyStatuses_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(Empty)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(FQDNProxyAgentServer).SubscribeProxyStatuses(m, &grpc.GenericServerStream[Empty, ProxyStatus]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FQDNProxyAgent_SubscribeProxyStatusesServer = grpc.ServerStreamingServer[ProxyStatus]
...
var FQDNProxyAgent_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SubscribeProxyStatuses",
			Handler:       _FQDNProxyAgent_SubscribeProxyStatuses_Handler,
			ServerStreams: true,
		},
		{
