...
func (p *policyWatcher) updateToServicesPolicies(ev serviceEvent) error {
	...
	if ev.previous != nil && ev.labels.Equals(ev.previous.labels) {
		...
		candidatePolicyKeys = p.inpByServiceID[ev.name]
	}
	...
	for key := range candidatePolicyKeys {
		...
		if !(p.inpMatchesService(inp, ev) ||
			(ev.previous != nil && p.inpMatchesService(inp, *ev.previous))) {
			continue
		}
...
func (p *policyWatcher) resolveToServices(key resource.Key, inp *types.SlimINP) {
	...
	for svc := range p.services.All(txn) {
		...
		if numMatches > 0 {
			p.markINPForService(key, svc.Name)
		} else {
			p.clearINPForService(key, svc.Name)
		}
	}
}
...
// inpMatchesService returns true if the cnp contains a ToServices rule which
// matches the provided service svcID/svc
func (p *policyWatcher) inpMatchesService(inp *types.SlimINP, ev serviceEvent) bool {
	if hasMatchingToServices(inp.Spec, ev) {
		return true
	}
...
func (p *policyWatcher) markINPForService(key resource.Key, svcID loadbalancer.ServiceName) {
...
func (p *policyWatcher) clearINPForService(key resource.Key, svcID loadbalancer.ServiceName) {
...
func hasToServices(inp *types.SlimINP) bool {
	...
	return slices.ContainsFunc(inp.Specs, specHasToServices)
}
