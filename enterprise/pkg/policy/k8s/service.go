...
func (p *policyWatcher) onServiceEvent(event k8s.ServiceNotification) {
	...
	if err != nil {
		p.log.Warn(
			"Failed to recalculate IsovalentNetworkPolicy rules after service event",
			logfields.Error, err,
...
func (p *policyWatcher) updateToServicesPolicies(svcID k8s.ServiceID, newSVC, oldSVC *k8s.MinimalService, newEps, oldEps *k8s.MinimalEndpoints) error {
	...
	if !(newService || changedService) {
		...
		candidatePolicyKeys = p.inpByServiceID[svcID]
	}
	...
	for key := range candidatePolicyKeys {
		...
		if !(p.inpMatchesService(inp, svcID, newSVC) ||
			(!newService && changedService && p.inpMatchesService(inp, svcID, oldSVC))) {
			continue
		}
		...
		resourceID := resourceIDForIsovalentNetworkPolicy(key, inp)

		errs = append(errs, p.resolveIsovalentNetworkPolicyRefs(inp, key, initialRecvTime, resourceID, nil))
	}
	return errors.Join(errs...)
}

// resolveToServices translates all ToServices rules found in the provided INP
// and to corresponding ToCIDRSet rules. Mutates the passed in INP in place.
func (p *policyWatcher) resolveToServices(key resource.Key, inp *types.SlimINP) {
	...
	p.svcCache.ForEachService(func(svcID k8s.ServiceID, svc *k8s.MinimalService, eps *k8s.MinimalEndpoints) bool {
		...
		numMatches := svcEndpoints.processRule(inp.Spec)
		for _, spec := range inp.Specs {
...
// inpMatchesService returns true if the INP contains a ToServices rule which
// matches the provided service svcID/svc
func (p *policyWatcher) inpMatchesService(inp *types.SlimINP, svcID k8s.ServiceID, svc *k8s.MinimalService) bool {
	if svc == nil {
		return false
	}

	if hasMatchingToServices(inp.Spec, svcID, svc) {
		return true
	}
...
// markINPForService marks that a policy (referred to by 'key') contains a
// ToServices selector selecting the service svcID
func (p *policyWatcher) markINPForService(key resource.Key, svcID k8s.ServiceID) {
	svcMap, ok := p.inpByServiceID[svcID]
	if !ok {
		svcMap = make(map[resource.Key]struct{}, 1)
		p.inpByServiceID[svcID] = svcMap
	}

	svcMap[key] = struct{}{}
}

// clearINPForService indicates that a policy (referred to by 'key') no longer
// selects the service svcID via a ToServices rule
func (p *policyWatcher) clearINPForService(key resource.Key, svcID k8s.ServiceID) {
	delete(p.inpByServiceID[svcID], key)
	if len(p.inpByServiceID[svcID]) == 0 {
		delete(p.inpByServiceID, svcID)
	}
}

// specHasMatchingToServices returns true if the rule contains a ToServices rule which
// matches the provided service svcID/svc
func hasMatchingToServices(spec *v1alpha1.IsovalentNetworkPolicyRule, svcID k8s.ServiceID, svc *k8s.MinimalService) bool {
	if spec == nil {
		return false
	}
...
// hasToServices returns true if the INP contains a ToServices rule
func hasToServices(inp *types.SlimINP) bool {
	if specHasToServices(inp.Spec) {
		return true
	}
	for _, spec := range inp.Specs {
