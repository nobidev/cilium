...
func TestT2HealthCheckCustomPortHTTP(t T) {
	testName := "t2-healthcheck-custom-port-http"
	...
	scenario.addBackendApplications(2, backendApplicationConfig{
		h2cEnabled:      true,
		listenPort:      8181,
		healthCheckPort: uint32(healthCheckPort),
	})
	...
	backends := []backendPoolOption{}
	for _, b := range scenario.backendApps {
		backends = append(backends, withIPBackend(b.ipv4, b.port), withHealthCheckPort(healthCheckPort))
	}
	backendPool := lbBackendPool(testName, backends...)
	scenario.createLBBackendPool(backendPool)
...
func TestT2HealthCheckTCP(t T) {
	...
	t.Log("Creating backend apps...")
	scenario.addBackendApplications(2, backendApplicationConfig{h2cEnabled: true, envVars: map[string]string{"TCP_ONLY_ENABLED": "true"}})
...
func TestT2HealthCheckCustomPortTCP(t T) {
	testName := "t2-healthcheck-custom-port-tcp"
