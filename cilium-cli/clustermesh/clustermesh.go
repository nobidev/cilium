...
func updateCABundleInValues(clusterName string, release *release.Release, clustersCA map[string]string) error {
	...
	if err != nil {
		return fmt.Errorf("existing tls.caBundle values are invalid for cluster %s", clusterName)
	}
	...
	if content == newContent && caBundleEnabled {
		return nil
	}

	caBundle, _, err := unstructured.NestedMap(release.Config, "tls", "caBundle")
	if err != nil {
		return fmt.Errorf("existing tls.caBundle values are invalid for cluster %s", clusterName)
	}
	if caBundle == nil {
		caBundle = make(map[string]any)
	}
	caBundle["enabled"] = true
	caBundle["content"] = newContent
	unstructured.SetNestedMap(release.Config, caBundle, "tls", "caBundle")
	...
	clustermeshApiserverAnnotations, _, err := unstructured.NestedMap(release.Config, "clustermesh", "apiserver", "podAnnotations")
	if err != nil {
		return fmt.Errorf("existing clustermesh.apiserver.podAnnotations values are invalid for cluster %s", clusterName)
	}
	if clustermeshApiserverAnnotations == nil {
		clustermeshApiserverAnnotations = make(map[string]any)
	}
	clustermeshApiserverAnnotations["cilium.io/caBundleChangeRestartedAt"] = time.Now().Format(time.RFC3339)
	unstructured.SetNestedMap(release.Config, clustermeshApiserverAnnotations, "clustermesh", "apiserver", "podAnnotations")
