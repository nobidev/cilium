...
func getRelease(kc *k8s.Client, params Parameters) (chart.Accessor, error) {
	rel, err := kc.HelmActionConfig.Releases.Last(params.HelmReleaseName)
	if err != nil {
		return nil, err
	}
	return chart.NewAccessor(rel)
}
...
func processLocalClient(localRelease chart.Accessor) (*ClusterState, error) {
	state := &ClusterState{
...
func setClustersInValues(release chart.Accessor, client *k8s.Client, clusters any) error {
...
func updateCABundleInValues(clusterName string, release chart.Accessor, clustersCA map[string]string) error {
	values := release.Values()
