...
#ifdef NORTH_SOUTH_TEST
	BUF_DECL(LB6_NS_EXTERNAL_IP, lb6_ns_external_ip);
	BUILDER_PUSH_BUF(builder, LB6_NS_EXTERNAL_IP);
#else
	BUF_DECL(LB6_EW_EXTERNAL_IP, lb6_ew_external_ip);
	BUILDER_PUSH_BUF(builder, LB6_EW_EXTERNAL_IP);
#endif
...
int lb6_external_ips_check(__maybe_unused const struct __ctx_buff *ctx)
{
	...
	BUF_DECL(LB6_NS_EXTERNAL_IP_POST_DNAT, lb6_ns_external_ip_post_dnat);
	ASSERT_CTX_BUF_OFF("lb6_ns_external_ip_post_dnat", "Ether", ctx, sizeof(__u32),
			   LB6_NS_EXTERNAL_IP_POST_DNAT, sizeof(BUF(LB6_NS_EXTERNAL_IP_POST_DNAT)));

	/* Ensure CT entry is updated accordingly (SVC). */
	ct_entry = map_lookup_elem(get_ct_map6(&tuple), &tuple);
	assert(ct_entry);
	assert(ct_entry->packets == 1);
	assert(ct_entry->bytes == sizeof(BUF(LB6_NS_EXTERNAL_IP_POST_DNAT)));
#else
	...
	ASSERT_CTX_BUF_OFF("lb6_ew_external_ip", "Ether", ctx, sizeof(__u32),
			   LB6_EW_EXTERNAL_IP, sizeof(BUF(LB6_EW_EXTERNAL_IP)));
