 ...
 * which triggers a stale conntrack entry (created for svc1) to be hit when
 * forwarding via svc2. The fix must update the CT entry's rev_nat_index to
 * svc2's, so the reply is RevNATed back to svc2's frontend, not svc1's.
 *
 * State from the preceding tests provides the stale svc1 CT/SNAT entries for
 * {CLIENT_IP:CLIENT_PORT -> BACKEND_IP_REMOTE:BACKEND_PORT}.
 */

/* Test that a request to svc2 (FRONTEND_IP_REMOTE_2), which shares the same
 * backend as svc1 (FRONTEND_IP_REMOTE), is correctly forwarded.
 */
...
/* Test that the reply from the shared backend is RevNATed to svc2's frontend
 * (FRONTEND_IP_REMOTE_2), not svc1's (FRONTEND_IP_REMOTE).
 */
