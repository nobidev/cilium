...
#define __add_egressgw_ha_policy_entry(_saddr, _daddr, _pfix_len, _size, _gateway_ips,		\
				       _egress_ip, _egress_ifindex)				\
{												\
...
#define add_egressgw_ha_policy_entry(_saddr, _daddr, _cidr, _size, _gateway_ips, _egress_ip,	\
				     _egress_ifindex)						\
	__add_egressgw_ha_policy_entry(_saddr, _daddr, EGRESS_PREFIX_LEN(_cidr), _size,		\
				       _gateway_ips, _egress_ip, _egress_ifindex)

#define add_egressgw_ha_default_policy_entry(_size, _gateway_ips, _egress_ip, _egress_ifindex)	\
	__add_egressgw_ha_policy_entry(0, 0, 0, _size, _gateway_ips, _egress_ip,		\
				       _egress_ifindex)

static __always_inline void
__del_egressgw_ha_policy_entry(__be32 saddr, __be32 daddr, __u32 prefixlen)
{
	struct egress_gw_ha_policy_key in_key = {
		.lpm_key = { prefixlen, {} },
		.saddr   = saddr,
...
static __always_inline void
del_egressgw_ha_policy_entry(__be32 saddr, __be32 daddr, __u8 cidr)
{
	__del_egressgw_ha_policy_entry(saddr, daddr, EGRESS_PREFIX_LEN(cidr));
}

static __always_inline void
del_egressgw_ha_default_policy_entry()
{
	__del_egressgw_ha_policy_entry(0, 0, 0);
}
