...
```
      ...
      --envoy-health-check-event-server-enabled                          Enables and deploys the Envoy health check event server that receives health check events from the node-local Envoy proxy
      --envoy-health-check-event-server-gc-interval duration             Interval of the Envoy health check table garbage collection (default 30s)
      ...
      --hubble-export-timescape-aggregation strings                      Perform aggregation pre-storage ('connection', 'identity')
      --hubble-export-timescape-aggregation-ignore-source-port           Ignore source port during aggregation (default true)
      ...
      --loadbalancer-flow-logs-sender-ipfix-collector-address string     Comma-separated list of IP:port, [IPv6]:port, or DNS:port addresses for the IPFix collector
