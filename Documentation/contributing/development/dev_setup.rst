...
Raise the ``golangci-lint`` ceiling in the same change rather than afterwards. On
every branch that carries a ``.custom-gcl.yaml``, the lint job compiles
``golangci-lint`` from source to produce the plugin binary, using the Go toolchain
the branch installs, so a ``golangci-lint`` release that requires a newer Golang
than the branch targets makes that job fail before it has linted anything. The two
ceilings cannot be merged into one Renovate rule, because ``allowedVersions`` takes
a single version range and the two dependencies are on different version series.
