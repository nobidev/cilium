...
func (n *NodesPodCIDRManager) reuseIPNets(
	...
	if canAllocateIPv4PodCIDRs && len(n.v4CIDRAllocators) != 0 {
		...
		if v4Err != nil {
			...
			log.WithError(v4Err).Warning("Duplicate v4 CIDR, will be released from this node")
		} else {
	...
	if canAllocateIPv6PodCIDRs && len(n.v6CIDRAllocators) != 0 {
		...
		if v6Err != nil {
			...
			log.WithError(v6Err).Warning("Duplicate v6 CIDR, will be released from this node")
		} else {
