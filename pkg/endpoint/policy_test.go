...
func newPolicyTestFixture(t *testing.T) *policyTestFixture {
	...
	repo := policy.NewPolicyRepository(logger, fakeAllocator.GetIdentityCache(), nil, nil, idManager, testpolicy.NewPolicyMetricsNoop())
	polComputer := compute.InstantiateCellForTesting(t, logger, "endpoint-policy_test", t.Name(), repo, idManager)
...
func TestStaleStatedbEntry(t *testing.T) {
	...
	res, _, _, found := f.polComputer.GetIdentityPolicyByNumericIdentity(f.podID.ID)
...
// recomputation can MaybeDetach the policy in that window.
func TestMaybeDetachRaceDuringRegeneration(t *testing.T) {
	...
	_, rev1 := f.repo.MustAddList(api.Rules{makeRule("r1", "peer", "80")})
	done, err := f.polComputer.RecomputeIdentityPolicy(f.podID, rev1)
	require.NoError(t, err)
	<-done
	...
	_, rev2 := f.repo.MustAddList(api.Rules{makeRule("r2", "peer2", "443")})
	done, err = f.polComputer.RecomputeIdentityPolicy(f.podID, rev2)
	require.NoError(t, err)
	<-done
	...
	_, rev3 := f.repo.MustAddList(api.Rules{makeRule("r3", "peer3", "8080")})
	go func() {
		time.Sleep(100 * time.Millisecond)
		done, err := f.polComputer.RecomputeIdentityPolicy(f.podID, rev3)
		if err == nil {
			<-done
		}
	}()
...
// TestSkippedPolicyRevision reproduces a race where a policy update arrives while
// the endpoint is already queued to regenerate. The duplicate trigger is skipped,
// but the queued regen must pick up the higher revision.
func TestSkippedPolicyRevision(t *testing.T) {
	...
	_, rev198 := f.repo.MustAddList(api.Rules{makeRule("rule-198")})
	_, rev199 := f.repo.MustAddList(api.Rules{makeRule("rule-199")})
	_, rev200 := f.repo.MustAddList(api.Rules{makeRule("rule-200")})

	newEP := func() *Endpoint {
		ep := &Endpoint{
			policyRepo:       f.repo,
			policyFetcher:    f.polComputer,
			desiredPolicy:    policy.NewEndpointPolicy(hivetest.Logger(t), f.repo),
			labels:           labels.NewOpLabels(),
			SecurityIdentity: f.podID,
			identityManager:  f.idManager,
	...
	t.Run("capture and propagate", func(t *testing.T) {
		// PolicyUpdate for rev199 is skipped while the endpoint is queued at rev198.
		ep := newEP()

		computePolicyForEPAndWait(t, ep, f.polComputer, rev198)
		...
		computePolicyForEPAndWait(t, ep, f.polComputer, rev199)

		ep.state = StateRegenerating
		require.NoError(t, ep.regeneratePolicy(new(regenerationStatistics), ctx))
	})

	t.Run("lower revision not decremented", func(t *testing.T) {
	...
	t.Run("multiple skips tracks max", func(t *testing.T) {
		...
		computePolicyForEPAndWait(t, ep, f.polComputer, rev200)
		ep.state = StateRegenerating
		require.NoError(t, ep.regeneratePolicy(new(regenerationStatistics), ctx))
	})

	t.Run("reset after consume", func(t *testing.T) {
	...
	t.Run("context already higher", func(t *testing.T) {
	...
	t.Run("fresh regen no skipped revision", func(t *testing.T) {
