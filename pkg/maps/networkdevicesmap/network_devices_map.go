...
type networkDeviceMap struct {
	*bpf.Map
}

func newNetworkDeviceMap() *networkDeviceMap {
	var index Index
...
func (m *networkDeviceMap) Upsert(ifindex uint32, state DeviceState) error {
	key := Index(ifindex)
	return m.Map.Update(&key, &state)
}

func (m *networkDeviceMap) Lookup(ifindex uint32) (*DeviceState, error) {
	key := Index(ifindex)
...
func (m *networkDeviceMap) IterateWithCallback(cb IterateCallback) error {
...
func (m *networkDeviceMap) Clear(ifindex uint32) error {
	key := Index(ifindex)
	return m.Map.Update(&key, &DeviceState{})
}

func (m *networkDeviceMap) init() error {
...
func (m *networkDeviceMap) close() error {
