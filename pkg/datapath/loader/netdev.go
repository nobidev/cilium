...
func removeObsoleteNetdevPrograms(logger *slog.Logger, devices []string, lnc *datapath.LocalNodeConfiguration) error {
	...
	for _, l := range links {
		...
		// per-endpoint maps.
		bpffsPath := lnc.BPFFSPaths.DeviceDir(l)
		...
		statePath := lnc.BPFFSPaths.StateDeviceDir(l.Attrs().Name)
