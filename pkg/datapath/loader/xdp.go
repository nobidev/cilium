...
// during tests.
func maybeUnloadObsoleteXDPPrograms(logger *slog.Logger, keep []string, xdpMode xdp.Mode, paths *datapath.BPFFSPaths) {
...
func loadAssignAttach(logger *slog.Logger, reg *registry.MapRegistry,
	...
	for i, spec := range xdpPermutations(spec) {
		commit, err = bpf.LoadAndAssign(logger, &obj, spec, &bpf.CollectionOptions{
			...
			ConfigDumpPath: filepath.Join(lnc.BPFFSPaths.StateDeviceDir(iface.Attrs().Name), xdpConfig),
		})
		if err != nil {
			return err
		}
		...
		err = attachXDPProgram(logger, iface, obj.Entrypoint, symbolFromHostNetdevXDP,
			lnc.BPFFSPaths.DeviceLinksDir(iface), xdpConfigModeToFlag(xdpMode))
...
// during tests.
func DetachXDP(ifaceName string, paths *datapath.BPFFSPaths, progName string) error {
