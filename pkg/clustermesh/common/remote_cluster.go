...
type RemoteCluster interface {
	...
	OnClusterIDChange(newID uint32)
	Stop()
	...
	rc.controllers.UpdateController(
		...
		controller.ControllerParams{
			...
			DoFunc: func(ctx context.Context) error {
				...
				if err := rc.onClusterConfigUpdate(config); err != nil {
...
func (rc *remoteCluster) onClusterConfigUpdate(config types.CiliumClusterConfig) error {
	...
	if err := rc.clusterIDsManager.ReserveClusterID(config.ID); err != nil {
		// release the old ID
		rc.RemoteCluster.OnClusterIDChange(types.ClusterIDUnset)
		rc.clusterIDsManager.ReleaseClusterID(rc.clusterID)
		rc.clusterID = types.ClusterIDUnset
		return err
	}

	rc.RemoteCluster.OnClusterIDChange(config.ID)
	rc.clusterIDsManager.ReleaseClusterID(rc.clusterID)
...
// a config updates signals the changed channel
func (rc *remoteCluster) watchClusterConfig(
	configUpdates <-chan types.CiliumClusterConfig, configChanged chan<- struct{},
) {
	for range configUpdates {
		// The config channels already deduplicated so we don't have to worry
		// about checking that here
		select {
		case configChanged <- struct{}{}:
		default:
			// The configChange channel is buffered with size one and only
			// relevant for one update, so if the channel is full we don't
			// have to send any event. This allow us to also drain the configs
			// channel safely.
		}
	}
}
