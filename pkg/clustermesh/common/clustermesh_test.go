...
type fakeRemoteCluster struct {
	...
	onClusterIDChange                      func(id uint32)
}
...
func (f *fakeRemoteCluster) OnClusterIDChange(id uint32) {
	if f.onClusterIDChange != nil {
		f.onClusterIDChange(id)
	}
}
...
func TestRemoteClusterIDLifecycle(t *testing.T) {
	...
	require.NoError(t, rc.onClusterConfigUpdate(types.CiliumClusterConfig{ID: 2}))
	...
	require.NoError(t, rc.onClusterConfigUpdate(types.CiliumClusterConfig{ID: 3}))
	...
	require.Error(t, rc.onClusterConfigUpdate(types.CiliumClusterConfig{ID: 4}))
	...
	require.NoError(t, rc.onClusterConfigUpdate(types.CiliumClusterConfig{ID: 4}))
	...
	require.Error(t, other.onClusterConfigUpdate(types.CiliumClusterConfig{ID: 4}))
