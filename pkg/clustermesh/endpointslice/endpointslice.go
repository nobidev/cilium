...
func (eps *ClusterEndpointSlice) Marshal() ([]byte, error) {
	data, err := eps.ClusterEndpointSliceEmbed.Marshal()
	if err != nil {
		return nil, err
	}
...
func (eps *ClusterEndpointSlice) Unmarshal(_ string, data []byte) error {
	newClusterEndpointSlice := ClusterEndpointSlice{}
	...
	if err := newClusterEndpointSlice.ClusterEndpointSliceEmbed.Unmarshal(data); err != nil {
		return err
	}

	if err := newClusterEndpointSlice.validate(); err != nil {
		return err
	}

	*eps = newClusterEndpointSlice

	return nil
}

// MarshalToJSON returns the cluster EndpointSlice object as JSON byte slice.
func (eps *ClusterEndpointSlice) MarshalJSON() ([]byte, error) {
	return json.Marshal(eps.ClusterEndpointSliceEmbed)
}

// UnmarshalFromJSON parses the JSON byte slice and updates the ClusterEndpointSlice receiver.
func (eps *ClusterEndpointSlice) UnmarshalJSON(data []byte) error {
	newClusterEndpointSlice := NewClusterEndpointSlice("", "")

	if err := json.Unmarshal(data, &newClusterEndpointSlice.ClusterEndpointSliceEmbed); err != nil {
		return err
	}
...
func (eps *ClusterEndpointSlice) validate() error {
	...
	if eps.ClusterID != option.Config.ClusterID {
		if err := cmtypes.ValidateClusterID(eps.ClusterID); err != nil {
			return err
		}
	}

	if eps.AddressType == "" {
		return errors.New("address type is unset")
	}
...
// NewClusterEndpointSlice returns a new cluster EndpointSlice definition
func NewClusterEndpointSlice(name, namespace string) ClusterEndpointSlice {
	return ClusterEndpointSlice{
		ClusterEndpointSliceEmbed: ClusterEndpointSliceEmbed{
			Name:        name,
			Namespace:   namespace,
...
type clusterEndpointSliceValidator func(key string, svc *ClusterEndpointSlice) error

func (vcs *ValidatingClusterEndpointSlice) Unmarshal(key string, data []byte) error {
	if err := vcs.ClusterEndpointSlice.Unmarshal(key, data); err != nil {
		return err
	}

	for _, validator := range vcs.validators {
		if err := validator(key, &vcs.ClusterEndpointSlice); err != nil {
			return err
		}
	}
...
func ClusterNameValidator(clusterName string) clusterEndpointSliceValidator {
	return func(_ string, svc *ClusterEndpointSlice) error {
...
func NamespacedNameValidator() clusterEndpointSliceValidator {
	return func(key string, svc *ClusterEndpointSlice) error {
