		...
		obs.store = params.StoreFactory.NewWatchStore(
			...
			&store.FuncObserver[*mcsapitypes.ValidatingMCSAPIServiceSpec]{
				OnUpdateFunc: func(obj *mcsapitypes.ValidatingMCSAPIServiceSpec) {
					if err := params.Cache.Update(&obj.MCSAPIServiceSpec); err != nil {
						params.Logger.Error(
							"Failed to update global service export cache",
							logfields.Error, err,
						)
					}
				...
				OnDeleteFunc: func(obj *mcsapitypes.ValidatingMCSAPIServiceSpec) {
					if err := params.Cache.Delete(&obj.MCSAPIServiceSpec); err != nil {
						params.Logger.Error(
							"Failed to delete from global service export cache",
							logfields.Error, err,
						)
					}
					params.Source.OnEvent(&obj.MCSAPIServiceSpec)
