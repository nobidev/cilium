...
func (s *adsServer) UpdateEnvoyResources(ctx context.Context, oldResources, newResources xds.Resources, waitGroup *completion.WaitGroup) error {
	...
	// If a listener exists in both old and new with the same address set, its port
	// allocation was already ACKed. Remove the callback to avoid double-ACKing it.
	for name, oldListener := range oldResources.Listeners {
		if newListener, found := newResources.Listeners[name]; found && listenerAddressesEqual(oldListener, newListener) {
	...
	restore := func(cause error) error {
		...
		if restoreErr != nil {
			return fmt.Errorf("%w; failed to restore ADS snapshot: %v", cause, restoreErr)
		}
		return cause
	}
