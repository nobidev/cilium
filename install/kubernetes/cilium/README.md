...
| authentication.mutual.spire.install.initImage | object | `{"digest":"sha256:ab33eacc8251e3807b85bb6dba570e4698c3998eca6f0fc2ccb60575a563ea74","override":null,"pullPolicy":"Always","repository":"docker.io/library/busybox","tag":"1.37.0","useDigest":true}` | init container image of SPIRE agent and server |
...
| enterprise.bgpControlPlane.nodeMaintenance.gracefulShutdownCommunity.enabled | bool | `false` | Enables sending of GRACEFUL_SHUTDOWN community when node is in maintenance mode (cordoned). |
| enterprise.bgpControlPlane.nodeMaintenance.withdrawTime | string | `"0s"` | Withdraws routes after configured timeout post going into maintenance mode. Does not withdraw if 0 / not configured. |
...
| enterprise.bgpControlPlane.statusReport.enabled | bool | `true` | Enable/Disable enterprise BGPv2 status reporting. It is recommended to enable status reporting in general, but if you have any issue such as high API server load, you can disable it by setting this to false. |
...
| envoy.image | object | `{"digest":"sha256:42347c3860964a1d8790c361356e082dbba1b0ded7b36243856182d1bc2a7ee0","override":null,"pullPolicy":"Always","repository":"quay.io/cilium/cilium-envoy","tag":"v1.35.2-1757248876-6e6f5d6763a6b437fc83c44943492d96d9c5c9aa","useDigest":true}` | Envoy container image. |
...
| hubble.export | object | `{"connectionlog":{"enabled":false,"exportInterval":"10s","fileCompress":true,"fileMaxBackups":5,"fileMaxSizeMb":10,"filePath":"/var/run/cilium/hubble/connectionlog.log"},"dynamic":{"config":{"configMapName":"cilium-flowlog-config","content":[{"aggregation":[],"aggregationIgnoreSourcePort":true,"aggregationRenewTTL":true,"aggregationStateFilter":["new","error","closed"],"aggregationTTL":"30s","excludeFilters":[],"fieldMask":[],"fileCompress":false,"fileMaxBackups":5,"fileMaxSizeMb":10,"filePath":"/var/run/cilium/hubble/events.log","fileRotationInterval":"0s","formatVersion":"v1","includeFilters":[],"name":"all","nodeName":null,"rateLimit":-1}],"createConfigMap":true},"enabled":false},"static":{"aggregation":null,"aggregationIgnoreSourcePort":null,"aggregationRenewTTL":null,"aggregationStateFilter":null,"aggregationTTL":null,"allowList":[],"denyList":[],"enabled":false,"fieldMask":[],"fileCompress":false,"fileMaxBackups":5,"fileMaxSizeMb":10,"filePath":"/var/run/cilium/hubble/events.log","fileRotationInterval":null,"formatVersion":"v1","overrideNodeName":null,"rateLimit":null},"timescape":{"aggregation":null,"aggregationIgnoreSourcePort":null,"aggregationRenewTTL":null,"aggregationStateFilter":null,"aggregationTTL":null,"allowList":null,"denyList":null,"enabled":null,"fieldMask":null,"maxBufferSize":null,"nodeName":null,"reportDroppedFlowsInterval":null,"target":null,"tls":{"ca":{"configMap":{"key":null,"name":null}},"enabled":null,"mtls":{"enabled":null,"secretName":null}}}}` | Hubble flows export. |
| hubble.export.connectionlog.enabled | bool | `false` | Enables experimental support for ConnectionLog export. @schema type: [boolean] @schema |
| hubble.export.connectionlog.exportInterval | string | `"10s"` | - Interval at which ConnectionLog are collected and written on disk. @schema type: [string] @schema |
| hubble.export.connectionlog.fileCompress | bool | `true` | - determines if the rotated files should be compressed. using gzip. @schema type: [boolean] @schema |
| hubble.export.connectionlog.fileMaxBackups | int | `5` | - The maximum number of exported files to retain. @schema type: [integer] @schema |
| hubble.export.connectionlog.fileMaxSizeMb | int | `10` | - The maximum size in megabytes of the log file before it gets rotated. @schema type: [integer] @schema |
| hubble.export.connectionlog.filePath | string | `"/var/run/cilium/hubble/connectionlog.log"` | - Absolute path of the ConnectionLog export file location. @schema type: [string] @schema |
| hubble.export.dynamic | object | `{"config":{"configMapName":"cilium-flowlog-config","content":[{"aggregation":[],"aggregationIgnoreSourcePort":true,"aggregationRenewTTL":true,"aggregationStateFilter":["new","error","closed"],"aggregationTTL":"30s","excludeFilters":[],"fieldMask":[],"fileCompress":false,"fileMaxBackups":5,"fileMaxSizeMb":10,"filePath":"/var/run/cilium/hubble/events.log","fileRotationInterval":"0s","formatVersion":"v1","includeFilters":[],"name":"all","nodeName":null,"rateLimit":-1}],"createConfigMap":true},"enabled":false}` | - Dynamic exporters configuration. Dynamic exporters may be reconfigured without a need of agent restarts. |
...
| hubble.export.dynamic.config.content | list | `[{"aggregation":[],"aggregationIgnoreSourcePort":true,"aggregationRenewTTL":true,"aggregationStateFilter":["new","error","closed"],"aggregationTTL":"30s","excludeFilters":[],"fieldMask":[],"fileCompress":false,"fileMaxBackups":5,"fileMaxSizeMb":10,"filePath":"/var/run/cilium/hubble/events.log","fileRotationInterval":"0s","formatVersion":"v1","includeFilters":[],"name":"all","nodeName":null,"rateLimit":-1}]` | -- Exporters configuration in YAML format. |
...
| hubble.export.static | object | `{"aggregation":null,"aggregationIgnoreSourcePort":null,"aggregationRenewTTL":null,"aggregationStateFilter":null,"aggregationTTL":null,"allowList":[],"denyList":[],"enabled":false,"fieldMask":[],"fileCompress":false,"fileMaxBackups":5,"fileMaxSizeMb":10,"filePath":"/var/run/cilium/hubble/events.log","fileRotationInterval":null,"formatVersion":"v1","overrideNodeName":null,"rateLimit":null}` | - Static exporter configuration. Static exporter is bound to agent lifecycle. |
| hubble.export.static.aggregation | string | `nil` | - Defines the aggregation to perform pre-storage ('connection', 'identity'). Leave empty to disable aggregation. @schema type: [null, array] @schema |
| hubble.export.static.aggregationIgnoreSourcePort | string | `true` | - Defines whether to ignore source port during aggregation. @schema type: [null, boolean] @schema |
| hubble.export.static.aggregationRenewTTL | string | `true` | - Defines whether to renew the flow TTL when a new flow is observed. @schema type: [null, boolean] @schema |
| hubble.export.static.aggregationStateFilter | string | `["new", "error", "closed"]` | - Defines the state changes to include while aggregating ('new', 'established', 'first_error', 'error', 'closed'). @schema type: [null, array] @schema |
| hubble.export.static.aggregationTTL | string | `30s` | - Defines the TTL for flow aggregation. @schema type: [null, string] @schema |
...
| hubble.export.static.fileRotationInterval | string | `0s` | - Defines interval at which to rotate output file (15s, 5m, 1h). Leave unset or set to 0s to disable time-based file rotation. @schema type: [null, string] @schema |
| hubble.export.static.formatVersion | string | `"v1"` | - Defines API format used to encode output file. Set to '' to use legacy format. |
| hubble.export.static.overrideNodeName | string | `nil` | - Override the node_name field in exported events. @schema type: [null, string] @schema |
| hubble.export.static.rateLimit | string | `-1` | - Defines the amount of events to allow per minute before being rate-limited. Leave unset or set to -1 to disable rate-limiting. @schema type: [null, integer] @schema |
| hubble.export.timescape.aggregation | string | `nil` | - Defines the aggregation to perform pre-storage ('connection', 'identity'). Leave empty to disable aggregation. @schema type: [null, array] @schema |
| hubble.export.timescape.aggregationIgnoreSourcePort | string | `true` | - Defines whether to ignore source port during aggregation. @schema type: [null, boolean] @schema |
| hubble.export.timescape.aggregationRenewTTL | string | `true` | - Defines whether to renew the flow TTL when a new flow is observed. @schema type: [null, boolean] @schema |
| hubble.export.timescape.aggregationStateFilter | string | `["new", "error", "closed"]` | - Defines the state changes to include while aggregating ('new', 'established', 'first_error', 'error', 'closed'). @schema type: [null, array] @schema |
| hubble.export.timescape.aggregationTTL | string | `30s` | - Defines the TTL for flow aggregation. @schema type: [null, string] @schema |
| hubble.export.timescape.allowList | string | `nil` | - The list of allow filters to apply to exported events. Leave empty to not apply any filters.  Each filter is a JSON object with the field to filter on as key and the values to match as value. The filter is applied as an AND operation.  @schema type: [null, array] @schema |
| hubble.export.timescape.denyList | string | `nil` | - The list of deny filters to apply to exported events. Leave empty to not apply any filters.  Each filter is a JSON object with the field to filter on as key and the values to match as value. The filter is applied as an AND operation.  @schema type: [null, array] @schema |
| hubble.export.timescape.enabled | string | `nil` | Enable the Hubble timescape exporter. @schema type: [null, boolean] @schema |
| hubble.export.timescape.fieldMask | string | `nil` | The list of fields to include in exported events. Leave empty to include all fields. @schema type: [null, array] @schema |
...
| hubble.export.timescape.tls.enabled | string | `nil` | Enable client TLS for the Hubble timescape exporter. @schema type: [null, boolean] @schema |
| hubble.export.timescape.tls.mtls.enabled | string | `nil` | Enable mTLS for the Hubble timescape exporter. @schema type: [null, boolean] @schema |
| hubble.export.timescape.tls.mtls.secretName | string | `nil` | The name of the Secret containing the TLS material to enable mTLS. The secret must be of type `kubernetes.io/tls`. @schema type: [null, string] @schema |
| hubble.k8sExporter | object | `{"affinity":{},"enabled":null,"export":{"fileCompression":true,"filePath":"/var/run/cilium/hubble/k8s-events.log","fileRotationInterval":"0s","maxFileBackups":3,"maxFileSize":50},"image":{"digest":"sha256:35dc2075221587365ab3930d7ad1029f07dff560cc705650b85d8d73846f4b18","override":null,"pullPolicy":"Always","repository":"quay.io/isovalent/hubble-k8s-exporter","tag":"v1.8.1","useDigest":true},"nodeSelector":{"kubernetes.io/os":"linux"},"podAnnotations":{},"podSecurityContext":{"fsGroup":65532},"priorityClassName":"","resources":{},"securityContext":{"capabilities":{"drop":["ALL"]},"runAsUser":0},"tolerations":[]}` | Enables experimental support for using the Hubble Kubernetes Events Exporter. The exporter is enabled by default when using integrated Hubble Timescape |
| hubble.k8sExporter.affinity | object | `{}` | Affinity for hubble-k8s-exporter |
| hubble.k8sExporter.nodeSelector | object | `{"kubernetes.io/os":"linux"}` | Node labels for pod assignment ref: https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector |
| hubble.k8sExporter.podAnnotations | object | `{}` | Annotations to be added to hubble-k8s-exporter pods |
| hubble.k8sExporter.resources | object | `{}` | Specifies the resources for the hubble-k8s-exporter pods |
| hubble.k8sExporter.securityContext | object | `{"capabilities":{"drop":["ALL"]},"runAsUser":0}` | hubble-k8s-exporter container security context |
| hubble.k8sExporter.tolerations | list | `[]` | Node tolerations for pod assignment on nodes with taints ref: https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/ |
...
| hubble.rbac | object | `{"auth":{"oidc":{"ca":{"configMap":{"key":"ca.crt","name":null}},"clientID":null,"issuerUrl":null}},"enabled":false,"extraArgs":{},"extraEnv":[],"healthCheckPort":8079,"image":{"digest":"sha256:30b13744a41efbe2849e0a7e47bade03f18a61c58ef9394f5b0ea785ac19e17f","override":null,"pullPolicy":"Always","repository":"quay.io/isovalent/hubble-rbac","tag":"v1.10.5","useDigest":true},"listenPort":4245,"loggingLevel":"info","policy":{"configMap":{"bindings":[],"create":true,"key":"hubble-rbac-policy.yaml","name":"hubble-rbac-policy","roles":[]},"logRoles":false},"resources":{},"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"privileged":false,"runAsGroup":65532,"runAsNonRoot":true,"runAsUser":65532},"useKubernetesGrpcProbes":true}` | Enables experimental support for using Hubble RBAC with Hubble Relay. This is incompatible with Hubble RBAC installed via the Hubble Enterprise chart. |
| hubble.rbac.enabled | bool | `false` | Enables hubble-rbac by running it as a sidecar in hubble-relay. |
| hubble.rbac.policy | object | `{"configMap":{"bindings":[],"create":true,"key":"hubble-rbac-policy.yaml","name":"hubble-rbac-policy","roles":[]},"logRoles":false}` | Options to configure the Hubble RBAC authorization policy. The policy can be defined inline in the configMap.bindings and configMap.roles field, or in a separate configMap specified by the configMap.name. |
| hubble.rbac.policy.configMap.bindings | list | `[]` | The list of bindings between claims and roles. |
| hubble.rbac.policy.configMap.create | bool | `true` | Whether to create the config map from where the mapping between claims and roles is read. If set to 'false' the user must create the config map out-of-band. Hubble Relay will not start until the config map is in place. |
...
| hubble.rbac.policy.configMap.roles | list | `[]` | The list of roles to define. |
...
| hubble.rbac.useKubernetesGrpcProbes | bool | `true` | Whether to use native Kubernetes gRPC-based probes (startup, liveness, readiness) which are stable since Kubernetes 1.27. If false, fallback to tcpSocket probes. |
...
| hubble.timescape | object | `{"affinity":{},"analyzer":{"enabled":false,"scheduleInterval":"24h"},"clickhouse":{"image":{"digest":"sha256:157423fca52f596b62a1bf301b037b9f74bfca38089aa57d1bd36c9025f7ecef","override":null,"pullPolicy":"Always","repository":"quay.io/isovalent/clickhouse-server","tag":"25.3.6.56-alpine","useDigest":true},"resources":{},"rollOutPods":true,"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"privileged":false,"runAsGroup":101,"runAsNonRoot":true,"runAsUser":101}},"clustermesh":{"primary":{"createNamespace":false,"id":0,"namespace":""}},"enabled":false,"image":{"digest":"sha256:d29ed064272a6d25b67f206b9716e334a13ae67e630542b32a1e46d6cae2df20","override":null,"pullPolicy":"Always","repository":"quay.io/isovalent/hubble-timescape-lite","tag":"v1.8.1","useDigest":true},"ingester":{"k8sImporter":{"enabled":false,"flushInterval":"5s"},"streamAPI":{"flushInterval":"5s"}},"logLevel":"info","metrics":{"clickhouse":{"enabled":true},"enabled":true,"serviceMonitor":{"annotations":{},"clickhouse":{"interval":"10s","metricRelabelings":null,"relabelings":null,"scrapeTimeout":null},"enabled":false,"interval":"10s","labels":{},"metricRelabelings":null,"relabelings":null,"scrapeTimeout":null}},"nodeSelector":{"kubernetes.io/os":"linux"},"persistence":{"enabled":false,"storageClassName":null,"volumeSize":"10Gi"},"podSecurityContext":{"fsGroup":65532},"priorityClassName":"","resources":{},"rollOutPods":false,"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"privileged":false,"runAsGroup":65532,"runAsNonRoot":true,"runAsUser":65532},"tls":{"client":{"existingSecret":""},"enabled":null,"server":{"existingSecret":"","extraDnsNames":[],"extraIpAddresses":[]}},"tolerations":[],"trimmer":{"flowCount":0,"intervalMinutes":20,"timout":null},"ttl":null,"ui":{"auth":{"enabled":null,"oidc":{"clientAssertionFile":null,"clientAssertionType":null,"clientID":null,"clientSecret":{"secretKey":"client-secret","secretName":"hubble-timescape-ui"},"extraFields":[],"idTokenUserClaim":"email","issuerCA":{"configMap":{"key":"ca.crt","name":null}},"issuerURL":null,"redirectURL":null,"scopes":["openid","email","offline_access"]},"sessionKey":{"secretKey":"session-key","secretName":"hubble-timescape-ui"}},"enabled":true,"ingress":{"annotations":{},"className":"","enabled":false,"hosts":["chart-example.local"],"labels":{},"tls":[]},"route":{"additionalRules":[],"annotations":{},"enabled":false,"filters":[],"hostnames":[],"labels":{},"matches":[{"path":{"type":"PathPrefix","value":"/"}}],"parentRefs":[]}},"useStreamAPI":false,"vector":{"affinity":{},"image":{"digest":"sha256:d185107307ddb3a713765a75caea45e81f85e079f2c1f9eb970e9b4bbdf7360f","override":null,"pullPolicy":"Always","repository":"quay.io/isovalent/vector","tag":"0.49.0-distroless-libc","useDigest":true},"metrics":{"enabled":true,"serviceMonitor":{"annotations":{},"enabled":false,"interval":"10s","labels":{},"metricRelabelings":null,"relabelings":null,"scrapeTimeout":null}},"nodeSelector":{"kubernetes.io/os":"linux"},"podSecurityContext":{"fsGroup":0},"priorityClassName":"","resources":{},"securityContext":{"allowPrivilegeEscalation":true,"capabilities":{"drop":["ALL"]},"privileged":true,"runAsGroup":0,"runAsNonRoot":false,"runAsUser":0},"tolerations":[]}}` | Enables support for integrated Hubble Timescape. |
| hubble.timescape.affinity | object | `{}` | Affinity for hubble-timescape |
| hubble.timescape.analyzer.enabled | bool | `false` | Enables the Hubble Timescape analyzer responsible for generating Network Security assessment reports. Note that k8sExporter must also be enabled for the analyzer to work properly as it relies on Cilium Identities. |
| hubble.timescape.analyzer.scheduleInterval | string | `"24h"` | The interval at which the analyzer will run and generate a new assessment report. When a run goes beyond this interval, the next one will start as soon as the running one is completed. |
| hubble.timescape.clickhouse.rollOutPods | bool | `true` | Roll out hubble-timescape pod when the clickhouse configmap is updated (hubble.timescape.rollOutPods must also be true). |
| hubble.timescape.clustermesh | object | `{"primary":{"createNamespace":false,"id":0,"namespace":""}}` | Experimental support for using Hubble Timescape with clustermesh. If enabled, all flow log will be sent to the `primary` Timescape instance. |
| hubble.timescape.ingester.k8sImporter.enabled | bool | `false` | *experimental* Whether to enable the k8s event watcher for the K8s Importer. When enabled, this effectively replaces the K8sExporter deployment. |
| hubble.timescape.ingester.k8sImporter.flushInterval | string | `"5s"` | The interval at which the K8s Importer should flush k8s events to the database. |
| hubble.timescape.ingester.streamAPI.flushInterval | string | `"5s"` | The interval at which the Stream API should flush flows to the database. |
| hubble.timescape.logLevel | string | `"info"` | Configure the log level. Valid values are: trace, debug, info, warn and error. |
| hubble.timescape.metrics.clickhouse.enabled | bool | `true` | Enables exporting timescape clickhouse metrics in OpenMetrics format. |
| hubble.timescape.metrics.enabled | bool | `true` | Enables exporting timescape metrics in OpenMetrics format. |
| hubble.timescape.metrics.serviceMonitor.annotations | object | `{}` | Annotations to add to ServiceMonitor |
| hubble.timescape.metrics.serviceMonitor.clickhouse.interval | string | `"10s"` | Interval for scrape metrics (Clickhouse metrics) |
| hubble.timescape.metrics.serviceMonitor.clickhouse.metricRelabelings | string | `nil` | Metrics relabeling configs for the ServiceMonitor (Clickhouse metrics) |
| hubble.timescape.metrics.serviceMonitor.clickhouse.relabelings | string | `nil` | Relabeling configs for the ServiceMonitor (Clickhouse metrics) |
| hubble.timescape.metrics.serviceMonitor.clickhouse.scrapeTimeout | string | `nil` | Timeout after which scrape is considered to be failed (Clickhouse metrics) |
| hubble.timescape.metrics.serviceMonitor.enabled | bool | `false` | Enable service monitor. This requires the prometheus CRDs to be available (see https://github.com/prometheus-operator/prometheus-operator/blob/main/example/prometheus-operator-crd/monitoring.coreos.com_servicemonitors.yaml) |
| hubble.timescape.metrics.serviceMonitor.interval | string | `"10s"` | Interval for scrape metrics |
| hubble.timescape.metrics.serviceMonitor.labels | object | `{}` | Labels to add to ServiceMonitor |
| hubble.timescape.metrics.serviceMonitor.metricRelabelings | string | `nil` | Metrics relabeling configs for the ServiceMonitor |
| hubble.timescape.metrics.serviceMonitor.relabelings | string | `nil` | Relabeling configs for the ServiceMonitor |
| hubble.timescape.metrics.serviceMonitor.scrapeTimeout | string | `nil` | Timeout after which scrape is considered to be failed. |
| hubble.timescape.nodeSelector | object | `{"kubernetes.io/os":"linux"}` | Node labels for pod assignment ref: https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector |
| hubble.timescape.persistence.enabled | bool | `false` | Enables persistent storage for Hubble Timescape using a PVC. If disabled, timescape will run on an ephemeral disk and all flows will be lost on restart. |
| hubble.timescape.rollOutPods | bool | `false` | Roll out Timescape pods automatically when configmap is updated. |
| hubble.timescape.tls.client.existingSecret | string | `""` | Name of the Secret containing the certificate and key for the Timescape PushAPI client. If specified, the certificate and key need to be managed separately and no certificate or key will be generated by the chart. |
| hubble.timescape.tls.server.existingSecret | string | `""` | Name of the Secret containing the certificate and key for the Hubble Timescape server. If specified, the certificate and key need to be managed separately and no certificate or key will be generated by the chart. |
...
| hubble.timescape.tolerations | list | `[]` | Node tolerations for pod assignment on nodes with taints ref: https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/ |
| hubble.timescape.trimmer | object | `{"flowCount":0,"intervalMinutes":20,"timout":null}` | When enabled, timescape will periodically trim the number of stored flows to the configured max number. NOTE: This is designed to act as a fallback to prevent storage overflow in exceptional cases and not as the main method to limit storage usage under normal operation. See `.hubble.timescape.ttl`. |
| hubble.timescape.trimmer.flowCount | int | `0` | Desired max number of flows stored. If 0 or null, trimming flows is disabled. |
| hubble.timescape.trimmer.intervalMinutes | int | `20` | Trimmer interval, in minutes. The trimmer function will run once per interval. Must be a positive integer. |
| hubble.timescape.trimmer.timout | string | `nil` | Trimmer timeout specified as a Go duration. Defaults to the configured Trimmer interval. Examples: 30s, 1m, 5m, 1h @schema type: [null, string] @schema |
| hubble.timescape.ttl | string | `nil` | Configures the time-to-live for flows. Flows older than the TTL are automatically pruned. This is the main method to limit storage usage under normal operation. Defaults to 6 hours or 1 hour if Timescape runs without persistent storage. @schema type: [null, string] @schema |
| hubble.timescape.ui.auth.oidc.clientAssertionFile | string | `nil` | The file containing the client assertion to use if clientAssertionType is set. @schema type: [null, string] @schema |
| hubble.timescape.ui.auth.oidc.clientAssertionType | string | `nil` | Client assertion type. Set to urn:ietf:params:oauth:client-assertion-type:jwt-bearer to use a JWT. @schema type: [null, string] @schema |
| hubble.timescape.ui.auth.oidc.clientID | string | `nil` | OIDC application client ID @schema type: [null, string] @schema |
| hubble.timescape.ui.auth.oidc.clientSecret | object | `{"secretKey":"client-secret","secretName":"hubble-timescape-ui"}` | OIDC application client secret |
| hubble.timescape.ui.auth.oidc.extraFields | list | `[]` | OIDC auth code redirect URL extra fields. Enrich the auth code redirect URL with extra fields in the query section. Such fields are sometimes required in environments that are using a non standard OIDC implementation. |
| hubble.timescape.ui.auth.oidc.idTokenUserClaim | string | `"email"` | OIDC user claim to extract from the id_token. The extracted value is used to uniquely identifier the user and perform authorization when RBAC is enabled. |
| hubble.timescape.ui.auth.oidc.issuerCA | object | `{"configMap":{"key":"ca.crt","name":null}}` | OIDC custom CA to verify against. Will default to `.hubble.rbac.auth.oidc.ca` when RBAC is enabled. |
| hubble.timescape.ui.auth.oidc.issuerURL | string | `nil` | OIDC Server discovery endpoint URL. e.g. https://my-account.okta.com/oauth2/default/.well-known/openid-configuration Will default to `.hubble.rbac.auth.oidc.issuerURL` when RBAC is enabled. @schema type: [null, string] @schema |
| hubble.timescape.ui.auth.oidc.redirectURL | string | `nil` | OID redirect URL. The URL to redirect to, after the application was authorized. This need to be set to URI_USED_TO_ACCESS_TIMESCAPE_UI/oauth2/callback. When the Timescape UI is exposed through `.hubble.timescape.ui.ingress`, the redirect URL will be populated automatically, using the first hostname @schema type: [null, string] @schema |
| hubble.timescape.ui.auth.oidc.scopes | list | `["openid","email","offline_access"]` | The OAuth2 scopes to send to the OIDC provider. |
| hubble.timescape.ui.auth.sessionKey | object | `{"secretKey":"session-key","secretName":"hubble-timescape-ui"}` | UI session key is used to encrypt the UI session state. Should be a randomly generated password with at least 16 characters. |
| hubble.timescape.ui.route.additionalRules | list | `[]` | Additional rules for the route. |
| hubble.timescape.ui.route.annotations | object | `{}` | Annotations to add to the route. |
| hubble.timescape.ui.route.enabled | bool | `false` | Whether to create a Gateway API 'HTTPRoute' resource. |
| hubble.timescape.ui.route.filters | list | `[]` | Filters to apply to the route. |
| hubble.timescape.ui.route.hostnames | list | `[]` | Hostnames for the route. |
| hubble.timescape.ui.route.labels | object | `{}` | Labels to add to the route. |
| hubble.timescape.ui.route.matches | list | `[{"path":{"type":"PathPrefix","value":"/"}}]` | Path matches for the route. |
| hubble.timescape.ui.route.parentRefs | list | `[]` | Parent references for the route. |
| hubble.timescape.useStreamAPI | bool | `false` | Use experimental Stream API for Hubble Timescape. This automatically enables the Hubble Timescape exporter in Cilium and the Stream API in Hubble Timescape. |
| hubble.timescape.vector.metrics.enabled | bool | `true` | Enables exporting vector metrics in OpenMetrics format. |
| hubble.timescape.vector.metrics.serviceMonitor.annotations | object | `{}` | Annotations to add to ServiceMonitor |
| hubble.timescape.vector.metrics.serviceMonitor.enabled | bool | `false` | Enable service monitor. This requires the prometheus CRDs to be available (see https://github.com/prometheus-operator/prometheus-operator/blob/main/example/prometheus-operator-crd/monitoring.coreos.com_servicemonitors.yaml) |
| hubble.timescape.vector.metrics.serviceMonitor.interval | string | `"10s"` | Interval for scrape metrics |
| hubble.timescape.vector.metrics.serviceMonitor.labels | object | `{}` | Labels to add to ServiceMonitor |
| hubble.timescape.vector.metrics.serviceMonitor.metricRelabelings | string | `nil` | Metrics relabeling configs for the ServiceMonitor |
| hubble.timescape.vector.metrics.serviceMonitor.relabelings | string | `nil` | Relabeling configs for the ServiceMonitor |
| hubble.timescape.vector.metrics.serviceMonitor.scrapeTimeout | string | `nil` | Timeout after which scrape is considered to be failed. |
| hubble.timescape.vector.nodeSelector | object | `{"kubernetes.io/os":"linux"}` | Node labels for pod assignment ref: https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector |
| hubble.timescape.vector.tolerations | list | `[]` | Node tolerations for pod assignment on nodes with taints ref: https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/ |
...
| preflight.envoy.image | object | `{"digest":"sha256:42347c3860964a1d8790c361356e082dbba1b0ded7b36243856182d1bc2a7ee0","override":null,"pullPolicy":"Always","repository":"quay.io/cilium/cilium-envoy","tag":"v1.35.2-1757248876-6e6f5d6763a6b437fc83c44943492d96d9c5c9aa","useDigest":true}` | Envoy pre-flight image. |
