...
| authentication.mutual.spire.install.initImage | object | `{"digest":"sha256:2383baad1860bbe9d8a7a843775048fd07d8afe292b94bd876df64a69aae7cb1","override":null,"pullPolicy":"Always","repository":"docker.io/library/busybox","tag":"1.37.0","useDigest":true}` | init container image of SPIRE agent and server |
...
| enterprise.dnsProxyHA.enabled | bool | `false` | Enable agent support for DNS proxy HA.  This enables support for DNS proxy HA, but does not actually deploy the HA proxy. For lifecycle reasons, the HA proxy is deployed as a separate Helm chart. See the documentation for how to deploy the HA proxy. |
| enterprise.dnsProxyHA.offlineMode.enabled | bool | `false` | Enable agent support for DNS HA proxy offline updates  This enables support for the DNS HA proxy to write updates directly to BPF when the agent is down. This must also be enabled in the HA proxy; this setting only enables necessary support APIs in the agent. |
...
| hubble.export.static | object | `{"aggregation":null,"aggregationIgnoreSourcePort":null,"aggregationInterval":"0s","aggregationRenewTTL":null,"aggregationStateFilter":null,"aggregationTTL":null,"allowList":[],"denyList":[],"enabled":false,"fieldAggregate":[],"fieldMask":[],"fileCompress":false,"fileMaxBackups":5,"fileMaxSizeMb":10,"filePath":"/var/run/cilium/hubble/events.log","fileRotationInterval":null,"formatVersion":"v1","overrideNodeName":null,"rateLimit":null}` | - Static exporter configuration. Static exporter is bound to agent lifecycle. |
...
| hubble.export.timescape.useCiliumServiceResolver | string | `true` | Whether to enable the Cilium service resolver to resolve the IP address of the Hubble Timescape service. This is enabled by default and is required when Timescape is running in the same cluster as the Cilium agent. Disable this option if Hubble Timescape is running in a different cluster not using clustermesh and fallback to host DNS resolution. @schema type: [null, boolean] @schema |
...
| hubble.timescape.service.annotations | object | `{}` | Annotations to be added to the hubble-timescape service |
| hubble.timescape.service.labels | object | `{}` | Labels to be added to the hubble-timescape service |
| hubble.timescape.service.type | string | `"ClusterIP"` | Service type for the hubble-timescape service |
